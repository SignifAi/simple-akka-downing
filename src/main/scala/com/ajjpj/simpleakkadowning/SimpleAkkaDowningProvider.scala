...
private[simpleakkadowning] class DowningActor(stableInterval: FiniteDuration, decider: SurvivalDecider) extends Actor with ActorLogging {
	...
	override def receive = {
		...
		case msg @ CurrentClusterState(members, unreachable, _, _, _) =>
			...
			val upMembers = members.filter(_.status == MemberStatus.Up).map(m => ClusterMemberInfo(m.address, m.roles, m))
			state = ClusterState(upMembers, unreachable.map(_.uniqueAddress.address))
			triggerTimer()
		case msg @ MemberUp(m) =>
			log.debug("State: {} - Msg: {}", state, msg)
			//TODO is there a good and meaningful way to count 'weaklyUp' members?
			state = state.copy(
				upMembers = state.upMembers + ClusterMemberInfo(m.address, m.roles, m),
				unreachable = state.unreachable - m.address)
			triggerTimer()
		case msg @ MemberLeft(m) =>
			log.debug("State: {} - Msg: {}", state, msg)
			state = state.copy(
				upMembers = state.upMembers.filterNot(_.uniqueAddress == m.address),
				unreachable = state.unreachable - m.address)
			triggerTimer()
	...
	private def iAmResponsibleAction(clusterState: ClusterState) = {
		clusterState.sortedUpAndReachable.head.uniqueAddress == cluster.selfAddress
	}
