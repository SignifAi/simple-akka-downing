...
object SurvivalDecider {
	private val memberOrdering = new Ordering[ClusterMemberInfo] {
		override def compare(x: ClusterMemberInfo, y: ClusterMemberInfo) =
			Address.addressOrdering.compare(x.uniqueAddress, y.uniqueAddress)
	}

	case class ClusterMemberInfo(uniqueAddress: Address, roles: Set[String], member: Member)
	case class ClusterState(upMembers: Set[ClusterMemberInfo], unreachable: Set[Address]) {
